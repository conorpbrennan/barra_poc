...
manager's 13F from the warm HTTP cache, rebuilds the per-(Date, Manager, Position) MV with the same
per-manager as-of join the builder uses, and overwrites ONLY `MV` on the existing frame — Weight and
...
def main() -> int:
    ...
    # into the per-manager as-of calendar and it shadows the previous filing for the months after it
    ...
    extra = len(new) - len(merged.dropna(subset=["MV"]).drop_duplicates(["Date", "Manager", "Position"]))
    ...
    old = pos.set_index(["Date", "Manager", "Position"])["MV"]
    ratio = (merged.set_index(["Date", "Manager", "Position"])["MV"] / old).replace([np.inf], np.nan)
    ...
    merged = merged[["Date", "Manager", "Position", "Weight", "MV", "ADV"]]
    merged.to_parquet(path, index=False)
    s = merged[(merged.Manager == "Soros") & (merged.Date == "2026-06-30")]
