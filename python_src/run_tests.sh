...
echo
echo "=== style-drift attribution tests (unit always; integ needs :8010 + built artifact) ==="
BARRA_API="${BARRA_API:-http://127.0.0.1:8010}" $PY test_drift.py; r16=$?
echo
...
echo
echo "=== scoped Q&A tests (unit always; integ needs :8010; live needs RUN_LLM=1) ==="
BARRA_API="${BARRA_API:-http://127.0.0.1:8010}" $PY test_ask.py; r19=$?
echo
echo "=== multi-manager 13F builder tests (unit always, no backend/network) ==="
$PY test_build_frames.py; r20=$?
echo
echo "=== multi-manager cube tests (unit always -- builds a small local cube, no backend/network) ==="
$PY test_multi_manager_cube.py; r21=$?
echo
echo "=== manager-scoping invariant (integ needs :8010; vacuous on single-manager data) ==="
BARRA_API="${BARRA_API:-http://127.0.0.1:8010}" $PY test_manager_scoping.py; r22=$?
echo
[ $r1 -eq 0 ] && [ $r2 -eq 0 ] && [ $r3 -eq 0 ] && [ $r4 -eq 0 ] && [ $r5 -eq 0 ] && [ $r6 -eq 0 ] && [ $r7 -eq 0 ] && [ $r8 -eq 0 ] && [ $r9 -eq 0 ] && [ $r10 -eq 0 ] && [ $r11 -eq 0 ] && [ $r12 -eq 0 ] && [ $r13 -eq 0 ] && [ $r14 -eq 0 ] && [ $r15 -eq 0 ] && [ $r16 -eq 0 ] && [ $r17 -eq 0 ] && [ $r18 -eq 0 ] && [ $r19 -eq 0 ] && [ $r20 -eq 0 ] && [ $r21 -eq 0 ] && [ $r22 -eq 0 ] && echo "ALL GREEN" || echo "FAILURES (repo=$r1 ui=$r2 measures=$r3 analysis=$r4 limits=$r5 dq=$r6 backtest=$r7 trends=$r8 docs=$r9 stress=$r10 drawdown=$r11 whatif=$r12 universe=$r13 funnel=$r14 span=$r15 drift=$r16 whatchanged=$r17 liquidity=$r18 ask=$r19 build_frames=$r20 multi_manager_cube=$r21 manager_scoping=$r22)"
exit $(( r1 || r2 || r3 || r4 || r5 || r6 || r7 || r8 || r9 || r10 || r11 || r12 || r13 || r14 || r15 || r16 || r17 || r18 || r19 || r20 || r21 || r22 ))
