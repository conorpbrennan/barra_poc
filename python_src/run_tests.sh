...
echo
echo "=== multi-book cube tests (unit always -- builds a small local cube, no backend/network) ==="
$PY test_multi_book_cube.py; r21=$?
echo
echo "=== book-scoping invariant (integ needs :8010; vacuous on single-book data) ==="
BARRA_API="${BARRA_API:-http://127.0.0.1:8010}" $PY test_book_scoping.py; r22=$?
echo
[ $r1 -eq 0 ] && [ $r2 -eq 0 ] && [ $r3 -eq 0 ] && [ $r4 -eq 0 ] && [ $r5 -eq 0 ] && [ $r6 -eq 0 ] && [ $r7 -eq 0 ] && [ $r8 -eq 0 ] && [ $r9 -eq 0 ] && [ $r10 -eq 0 ] && [ $r11 -eq 0 ] && [ $r12 -eq 0 ] && [ $r13 -eq 0 ] && [ $r14 -eq 0 ] && [ $r15 -eq 0 ] && [ $r16 -eq 0 ] && [ $r17 -eq 0 ] && [ $r18 -eq 0 ] && [ $r19 -eq 0 ] && [ $r20 -eq 0 ] && [ $r21 -eq 0 ] && [ $r22 -eq 0 ] && echo "ALL GREEN" || echo "FAILURES (repo=$r1 ui=$r2 measures=$r3 analysis=$r4 limits=$r5 dq=$r6 backtest=$r7 trends=$r8 docs=$r9 stress=$r10 drawdown=$r11 whatif=$r12 universe=$r13 funnel=$r14 span=$r15 drift=$r16 whatchanged=$r17 liquidity=$r18 ask=$r19 build_frames=$r20 multi_book_cube=$r21 book_scoping=$r22)"
