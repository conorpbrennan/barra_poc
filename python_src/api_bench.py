...
def suite(manager: str, date: str) -> list[tuple[str, str, str, dict | None]]:
    ...
    return [
        ...
        ("risk_histfull",              "GET",  f"/risk?date={date}&set=HistFull&manager={manager}", None),
        ("limits",                     "GET",  f"/limits?date={date}&manager={manager}", None),
        ("dq",                         "GET",  "/dq", None),
        ("backtest",                   "GET",  f"/backtest?date={date}&manager={manager}", None),
        ("contributions",              "GET",  f"/contributions?date={date}&manager={manager}", None),
        ("whatchanged",                "GET",  f"/whatchanged?date={date}&manager={manager}", None),
        ("pnl_attribution",            "GET",  f"/pnl_attribution?manager={manager}", None),
        ("pnl_attribution_linkage",    "GET",  f"/pnl_attribution/linkage?manager={manager}", None),
        ("pnl_attribution_residual",   "GET",  f"/pnl_attribution/residual?manager={manager}", None),
        ("pnl_attribution_names",      "GET",  f"/pnl_attribution/names?manager={manager}", None),
        ("pnl_attribution_by_sector",  "GET",  f"/pnl_attribution?manager={manager}&by=sector", None),
        # ---- Trends lens
        ("trends_book",                "GET",  f"/trends?set=HistFull&manager={manager}", None),
        ("trends_book_modelvol",       "GET",  f"/trends?set=HistFull&manager={manager}&measures=Model%20vol", None),
        ("trends_by_factor",           "GET",  f"/trends?set=HistFull&manager={manager}&measures=Net%20exposure&by=Factor", None),
        # ---- Stress / What-if lenses
        ("stress_naive",               "POST", "/stress", {"shocks": {"Value": -2.0}, "date": date, "manager": manager}),
        ("stress_conditional",         "POST", "/stress", {"shocks": {"Value": -2.0}, "date": date, "manager": manager, "conditional": True}),
        ("reverse_stress",             "GET",  f"/reverse_stress?date={date}&manager={manager}", None),
        ("hedge",                      "GET",  f"/hedge?date={date}&manager={manager}", None),
        ("whatif_empty",               "POST", "/whatif", {"trades": [], "date": date, "manager": manager}),
        # ---- Universe / Drift lenses (artifact-backed; may be manager_mismatch for the large manager)
        ("universe",                   "GET",  f"/universe?manager={manager}", None),
        ("funnel",                     "GET",  f"/funnel?manager={manager}", None),
        ("span",                       "GET",  f"/span?manager={manager}", None),
        ("drift",                      "GET",  f"/drift?manager={manager}", None),
        # ---- Model lens
        ("calibration",                "GET",  f"/calibration?manager={manager}", None),
